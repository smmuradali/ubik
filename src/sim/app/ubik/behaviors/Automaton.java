...
public abstract class Automaton {
 ...
 //Boolean to indicate that the automate has finished. Finished indicates both stop and pause depending on whether someone returns to execute the automata or not.
 private boolean finished;
    private static final Logger LOG = Logger.getLogger(Automaton.class.getName());

 /**Imprimir evolución de automata por pantalla*/

 /** Remaining execution time. You can set a time for an automata or state
   * (different from the highest in the hierarchy) end even if it has a default state. With
   * -1 is ignored. In restart you have to put it to duration ..
  */
  ...
  /**Prioridad del estado o comportamientos del autómata en forma de entero. No hay valores especiales*/
    ...
    */
   //reference automation father but not mendatory
 ...
 /**
   ...
   public Automaton(Person personImplementingAutomaton, int priority, int duration, String name){
     ...
     * 
     * 
     * Add transition to a state in the list of pending transitions. The list
      * is ordered from highest to lowest priority.
  ...
  public int transitionsPlanned(){
    ...
    public void nextState(SimState simState) {
      ...
      if ((!pendingTransitions.isEmpty()) && pendingTransitions.getFirst().priority > currentState.priority) {
        ...
        //next step of the current state
        currentState.nextState(simState); //the states have their bez substates, it is hierarchical automata.

        //reduce current state time. With -1 it is indicated that the completion does not depend on time.
...
public static void setEcho(boolean b){
   ...
   if(b) LOG.setLevel(Level.FINEST);
    ...
    public void setPendingTransitions(LinkedList<Automaton> pendingTransitions) {
